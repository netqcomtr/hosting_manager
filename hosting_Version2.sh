...
manage_domain_menu() {
    ...
    print_header "Domain Yönetimi - $domain"
    ...
    while true; do
        ...
        echo "4) Servisleri Güncelle"
        echo "5) Servis Ekle"
        echo "6) Container Durumunu Görüntüle"
        echo "7) Logları Görüntüle"
        echo "8) Manuel Yedek Al"
        echo "9) Yedekten Geri Yükle"
        echo "10) Otomatik Yedekleme Ayarla"
        echo "11) Domain Bilgilerini Görüntüle"
        echo "0) Geri"
        
        menu_prompt
        read -p "Seçim: " domain_choice
        
        case $domain_choice in
            1) start_domain "$domain" ;;
            2) stop_domain "$domain" ;;
            3) restart_domain "$domain" ;;
            4) update_domain_services "$domain" ;;
            5) add_external_service "$domain" ;;
            6)
                cd "$DOMAINS_PATH/$domain" || true
            ...
            7)
                ...
                ;;
            8) backup_domain "$domain" ;;
            9) restore_domain_backup "$domain" ;;
            10) enable_automatic_backups "$domain" ;;
            11) show_installation_summary "$domain" ;;
            0) return ;;
            *) log_error "Geçersiz seçim" ;;
        esac
    done
...
backup_menu() {
    print_header "Yedekleme İşlemleri"
    
    echo -e "\n${CYAN}Mevcut Domainler:${NC}\n"
    ...
    print_header "Yedekleme - $selected_domain"
    echo -e "\n${CYAN}İşlemler:${NC}\n"
    echo "1) Manuel Yedek Al"
    echo "2) Yedekten Geri Yükle"
    echo "3) Yedekleri Listele"
    echo "4) Eski Yedekleri Temizle"
    echo "0) Geri"
    
    menu_prompt
    read -p "Seçim: " backup_choice
    
    case $backup_choice in
        1) backup_domain "$selected_domain" ;;
        2) restore_domain_backup "$selected_domain" ;;
        3)
            echo -e "\n${CYAN}Yedekler:${NC}\n"
            ls -lh "$DOMAINS_PATH/$selected_domain/backups/" 2>/dev/null || log_warning "Yedek bulunamadı"
            ;;
        4) cleanup_old_backups "$selected_domain" ;;
        0) return ;;
        *) log_error "Geçersiz seçim" ;;
    esac
}
